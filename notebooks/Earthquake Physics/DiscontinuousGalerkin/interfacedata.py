...
def friction_law(v_m,t_m,v_p,t_p,S,psi,rho_m,mu_m,rho_p,mu_p,alpha,Tau_0,fric_law):
    ...
    Phi = eta_s * (1.0/Zs_p * p_p - 1.0/Zs_m * q_m)  # stress transfer functional. traction on a locked fault
    ...
    if(fric_law in 'SW'):
        ...
        # tractions on the fault: stress changes
            T_m = Tau_h - Tau_0 
            T_p = Tau_h - Tau_0
...
def TauStrength(S):
    ...
    fric_coeff = mu_s - (mu_s-mu_d) * min(S,D_c)/D_c     # friction coefficient
...
def regula_falsi(V,Phi,eta,sigma_n,psi,V0,a):
    ...
    #solve: V + sigma_n/eta*f(V,psi) - Phi/eta = 0, for slip-rate V                                                                                              
    ...
    fv = f_v(V,Phi,eta,sigma_n,psi,V0,a)   #V + a*sigma_n/eta*math.asinh(0.5*V/V0*np.exp(psi/a)) - Phi/eta
    fl = f_v(Vl,Phi,eta,sigma_n,psi,V0,a)   #Vl + a*sigma_n/eta*math.asinh(0.5*Vl/V0*np.exp(psi/a)) - Phi/eta
    fr = f_v(Vr,Phi,eta,sigma_n,psi,V0,a)   #Vr + a*sigma_n/eta*math.asinh(0.5*Vr/V0*np.exp(psi/a)) - Phi/eta
...
def f_v(V,Phi,eta,sigma_n,psi,V0,a):
